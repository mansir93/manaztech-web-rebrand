...
export default function ClientStrip() {
  return (
    <section
      ...
      className="bg-muted/20 mt-5 py-14 sm:py-16"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <ul className="mt-8 grid grid-cols-2 gap-x-6 gap-y-8 sm:grid-cols-3 lg:grid-cols-6 lg:gap-x-8">
          {clients.map((name, i) => (
            ...
            >
              <span className="text-muted-foreground/50 hover:text-foreground text-center text-sm font-semibold tracking-tight grayscale transition-all duration-300 select-none hover:grayscale-0">
                {name}
              </span>
            </motion.li>
          ))}
